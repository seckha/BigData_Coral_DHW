...
# extract() -> extract values from a SpatRaster for a set of locations
  ...
  # xy = TRUE to keep lon and lat
ex_north <- terra::extract(stack_over4dhw, data.frame(north_reef$lon, north_reef$lat), xy = TRUE)
ex_central <- terra::extract(stack_over4dhw, data.frame(central_reef$lon, central_reef$lat), xy = TRUE)
ex_south <- terra::extract(stack_over4dhw, data.frame(south_reef$lon, south_reef$lat), xy = TRUE)
# maybe have to take out xy later
```
