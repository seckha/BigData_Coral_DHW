...
stack_four_dhw <- rast(four_dhw_list)
plot(stack_means$`2021`)
...
Mean DHW for each summer (1 value per location per year)
```{r}
...
over4dhw_df <- as.data.frame(stack_four_dhw, xy = TRUE)
...
```{r}
library(fuzzyjoin)
xoxo <- geo_join(corals, mean_dhw, unit = "km", max_dist = 5.55)
# 1° lon = 111 km -> 0.05*111 (because it can be 0.05° apart to find the next closest one)
# geo_join uses km

# 1° lat varies at every lat
```
...
but then use what ts package for coral data? how to conenct with dhw?
...
use extract function to get dhw for specific location
x = raster
y = coral lat and lon
-> gives me timeseries data
-> pick one or two to start

extract(rasterstack, c(lon,lat)) lon and lat are numbers for coordinates
extract(rasterstack, data.frame(x=10, y=10))
years(c(1992:2021)) gives dates and years
names(of extract object) == years(c(1992:2021))
as.Date(names(xx), format="X%Y") get X away from column name
...
ts object
correlate one ts with another
ts package might be enough
lubridate to convert years (columns) to date format
-> how to do time series regression in r (video)

choose coral locs based on how much data there is for cover (e.g. 20 data points)

use over4dhw, because might say more than mean that has been smoothed

plot all sectors instead of only north, central, south

animate over4dhw and create video (not necessarily with dot over top) -> have that in prez and have plots after another in report (raster stack images and run through)
